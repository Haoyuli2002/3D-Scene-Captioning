...
class Caption_Generator():
    def __init__(self, model_name="AIDC-AI/Ovis2-1B", summarization_name = "Qwen/Qwen1.5-0.5B-Chat"):
        ...
        llm = AutoModelForCausalLM.from_pretrained(self.model_id, device_map="auto", torch_dtype="auto")
        self.summarizer_pipe = pipeline("text-generation", model=llm, tokenizer=tokenizer)

    def object_level_caption_refined(self, images, max_partition=5):
        ## multiple-images input
        images = images
        max_partition = max_partition
        text = '''
        Your task is to write one **short but detailed** sentences describing only the main object.
        This object is surrounded by a green bounding box in the images.
        The description should focus on the object's distinguishing features: shape, color, texture, number of holes or buttons, any unique patterns or symbols. Avoid generic phrases like "a single object" or "rectangular shape" without detail.
    ...
    def object_level_caption(self, query, images, max_partition=5, summary=False, summary_prompt=None):
    ...
    def single_image_surrounding_captioning(self, images_with_surrounding = None, prompt = None):
        if prompt is None:
                prompt = '''
                    You are provided with multiple views of a specific object which is enclosed in a red bounding box. These views also include its immediate surroundings and neighboring objects.
                    ...
                    Now generate the caption:
                    '''
        query = '\n'.join([f'Image {i+1}: <image>' for i in range(len(images_with_surrounding))]) + '\n' + prompt
        caption = self.object_level_caption(query=query, images=images_with_surrounding, max_partition=len(images_with_surrounding))

...
def img_visualization(images = None, masks = None, mask_ids = None, masked_images = None):
    if images and masks and mask_ids:
        fig, axes = plt.subplots(3, 5, figsize=(12, 6))
        length = len(images)+len(masks)
        binary_masks = []
        for idx, ax in enumerate(axes.flat):
            if idx < len(images):
                ...
                img = np.array(masks[idx-length])
                object_id = mask_ids[idx-length]
                binary_mask = (img == object_id).astype(np.uint8) * 255
                if(len(binary_masks)<=5):
                    binary_masks.append(binary_mask)
                ax.imshow(binary_mask, cmap = 'gray')
                ax.axis('off')
                ax.set_title(f'Object_Masks {idx+1-length}')
        plt.tight_layout()
        plt.show()
    elif images is None and masked_images:
